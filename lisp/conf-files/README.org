...
Эта часть конфига посвященая настройки folding'а, свертки блоков кода или текста. Подробнее: [[./fold/README.org][README folding'а]]
#+begin_src emacs-lisp
(add-to-list 'load-path "~/.emacs.d/lisp/conf-files/fold/")
(require 'fold-config)
#+end_src
** Other
:PROPERTIES:
...
Эта часть подгружает настройки для работы с LaTex. Подробнее: [[./tex/README.org][README для tex]]
#+begin_src emacs-lisp
(add-to-list 'load-path "~/.emacs.d/lisp/conf-files/tex/")
(require 'tex-config)
#+end_src
** Note-Taking
:PROPERTIES:
...
:END:
Тут будет подключаться модуль для создания заметок (данная проблема настолько разрослась, что потребовался отдельный модуль).
#+begin_src emacs-lisp
(add-to-list 'load-path "~/.emacs.d/lisp/conf-files/note-taking/")
(require 'note-taking)
#+end_src
** AI
:PROPERTIES:
...
Это часть подключает экспериментальную часть для работы с базами данных. Подробнее: [[./db/README.org][README для db]].
#+begin_src emacs-lisp
(add-to-list 'load-path "~/.emacs.d/lisp/conf-files/db/")
(require 'db-config)
#+end_src
* Provide
:PROPERTIES:
