...
(use-package org-roam
  ...
  (org-roam-db-autosync-idle-delay 10)
...
(defun my/org-roam-refresh-agenda-list ()
  (interactive)
  (let ((tags '("Project" "TODO")))
    (dolist (elem tags)
      (let ((notes (my/org-roam-list-notes-by-tag elem)))
        (mapc (lambda (el) (add-to-list 'org-agenda-files el)) notes)))))
...
(transient-define-prefix my/org-roam-tmenu ()
  ...
  [["Node"
    ...
    ("I" "Create id for heading" org-id-get-create :transient t :if (lambda () (derived-mode-p 'org-mode)))
    ("c" "Capture" org-roam-capture :transient nil)
    ("e" "Extract subtree" org-roam-extract-subtree :transient nil :if (lambda () (derived-mode-p 'org-mode)))
    ("R" "Refile current (caution!!!)" org-roam-refile :transient nil :if (lambda () (derived-mode-p 'org-mode)))
...
(autoload 'my/org-roam-tmenu "org-roam-menu" "Transient menu for org-roam" t)
#+end_src
...
[:class transient-row
        (casual-lib-quit-one)])
(defun my/bibliography-tmenu ()
  (interactive)
  (require 'ebib)
  (my/bibliography-tmenu--))
#+end_src

** Vulpea-Test
:PROPERTIES:
:CUSTOM_ID: vulpea-test
:END:
#+begin_src emacs-lisp
(use-package vulpea
  :after org
...
*** Embark-Vulpea
:PROPERTIES:
:CUSTOM_ID: embark-vulpea
:END:
#+begin_src emacs-lisp
(use-package embark-vulpea
  :after vulpea
  :ensure (embark-vulpea :host github :repo "fabcontigiani/embark-vulpea"))
#+end_src

*** Vulpea-Capf
:PROPERTIES:
:CUSTOM_ID: vulpea-capf
:END:
#+begin_src emacs-lisp
(use-package vulpea-capf
  :after vulpea
  :ensure (vulpea-capf :host github :repo "neonmei/vulpea-capf")
