...
- [[#пакеты][Пакеты]]
  ...
  - [[#org-citar][Org-Citar]]
    ...
    - [[#org-noter-integration][Org-Noter Integration]]
- [[#provide][Provide]]
:END:
...
(use-package citar
  :custom
  (citar-bibliography '("~/.bib/test.bib") "List of bibliography files")
  (citar-library-paths '("~/Документы/Библиотека") "List of libraries direcotories (where pdf will be stored)")
...
(transient-define-prefix my/bibliography-tmenu ()
  ...
  [["Bibliographic notes"
   ...
   ["Ebib"
    ("s" "Start" ebib :if (lambda () (= 0 (length ebib--buffer-alist))) :transient nil)
...
:PROPERTIES:
:CUSTOM_ID: org-noter-integration
:END:
#+begin_src emacs-lisp
