...
:PROPERTIES:
:CUSTOM_ID: pixel-scroll-precision-mode
:END:
#+begin_src emacs-lisp
...
(load-file "/home/berlizoves/.emacs.d/themes/slaanesh-dark-theme.el")
(load-theme 'slaanesh-dark t)
#+end_src
...
(my/set-fonts)
(add-hook 'server-after-make-frame-hook (lambda () (run-with-idle-timer 2 nil #'my/set-fonts)))
(add-hook 'after-init-hook (lambda () (run-with-idle-timer 2 nil #'my/set-fonts)))
#+end_src
...
( :package "dashboard"
  ...
  :pin t
  :ref "9cfcb08c5623816d8f9a68a76c54845bc4bf46df"
  :source "MELPA")
  :init
  (setq dashboard-set-file-icons t)
  (setq dashboard-set-heading-icons t)
  (setq dashboard-display-icons-p t)
  (setq dashboard-image-banner-max-width 650)
  ...
  (dashboard-footer-messages '("Perfection is not a goal — it is your birthright. Code beautifully."
                               "Let your keystrokes dance like silk on marble. Emacs obeys only the exquisite."
                               "Every parenthesis closed is a note in the symphony of your will."
                               "You do not write code — you compose desire in syntax."
                               "The true path lies not in speed, but in the ecstasy of flawless expression."
                               "Embrace the pleasure of precision. Let no byte be unadorned."
                               "Your .emacs.d is your shrine — make it worthy of devotion."
                               "Suffering is for those who use GUIs. You? You ascend through key chords."
                               "In the silence between keystrokes, hear the whisper of perfection."
                               "You are not debugging — you are refining reality to match your vision."
                               "Let your code flow like nectar — pure, potent, and intoxicating."
                               "Only the worthy may wield M-: with grace. You are among them."))
  :config
...
(use-package prism
  ...
  :hook (
	 ((python-mode python-ts-mode html-mode vue-html-mode yaml-mode yaml-ts-mode) . prism-whitespace-mode)
	 ((javascript-mode js-ts-mode js-mode js2-mode css-mode typescript-mode typescript-ts-mode tsx-ts-mode json-mode yaml-mode html-mode markdown-mode  latex-mode bash-mode scheme-mode janet-mode janet-ts-mode go-mode go-ts-mode lisp-mode sh-mode bash-ts-mode qml-ts-mode lua-mode lua-ts-mode) . prism-mode)
	 )
  :mode
...
:PROPERTIES:
:CUSTOM_ID: color-identifiers-mode
:END:
#+begin_src emacs-lisp
(use-package color-identifiers-mode
  :hook
  ((python-ts-mode c-ts-mode bash-ts-mode) . color-identifiers-mode))
#+end_src

#+RESULTS:
: [nil 26962 21569 489051 nil elpaca-process-queues nil nil 754000 nil]
