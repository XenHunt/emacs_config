...
5. *Iterate for Perfection:*  
   ...
   Refined:  
   ,*Goal:* Simplify the concept of machine learning for a 10-year-old.  
   ,*Constraints:* Use analogies (e.g., 'like a child learning to recognize animals'), avoid equations, and limit to 150 words.  
   ,*Example Output:* 'Machine learning is like teaching a robot to identify cats. Every time you show it a cat picture, it learns new clues, until it can spot cats on its own!'  

   ,*Why This Works:*  
   This structure ensures the AI understands /exactly/ what to prioritize, reduces interpretive errors, and aligns the prompt with the model’s strengths. By defining boundaries and testing for contradictions, the AI avoids 'doublethink' and delivers precise, actionable results.
   ")
