...
:CUSTOM_ID: functions
:END:
#+begin_src emacs-lisp :tangle extra-ai.el
;; -*- lexical-binding: t;  no-byte-compile: t -*-
...
(use-package gptel
  :commands (gptel)
  :general
  (:states 'normal "SPC e g g" 'gptel)
  (:states 'normal "SPC e g m" 'gptel-menu)
  (:states 'normal "SPC e g c" 'gptel-add)
  (:states 'normal "SPC e g f" 'gptel-add-file)
  (:states 'normal "SPC e g a" 'gptel-abort)
  (:states 'normal "SPC e g s" 'gptel-send)
  :custom
  ...
  (setq
   ...
   '((markdown-mode . "[Response] ")
     (org-mode . "*** Response: ")
     (text-mode . "[Response]"))
   gptel-backend (gptel-make-openai "open-webui-local"
                   :host "127.0.0.1:3000"          ; Адрес Open WebUI
                   :endpoint "/api/chat/completions"           ; Специальный эндпоинт Open WebUI
                   :protocol "http"
                   :stream t
                   :models '("llama.cpp")             ; Имя модели, как она называется в Open WebUI
                   :key "sk-9e70ebebf57d4cf387e2521b1e0b4f69"))
  ...
  (gptel-make-openai "llama-cpp"          ;Any name
    ...
    :protocol "http"
    :host "localhost:8085"                ;Llama.cpp server location
...
Your role is to optimize and restructure prompts for other AI models to enhance their clarity, precision, and effectiveness. The goal is to ensure the revised prompts eliminate ambiguity, avoid contradictions, and align with the target model’s capabilities.  

,*Specific Instructions:/  
1. /Analyze the Original Prompt:/  
   - Identify vague phrases (e.g., 'as best as you can,' 'without doublethink').  
   - Detect assumptions or unspoken requirements (e.g., 'upgrade' without defining criteria).  

2. /Define Clear Objectives:/  
   - Specify the /goal/ of the prompt (e.g., 'Generate a step-by-step tutorial on Python loops').  
   - Clarify the /output format/ (e.g., bullet points, code blocks, numbered lists).  
   - Include /constraints/ (e.g., 'Avoid technical jargon for novice users').  

3. /Structure for Maximum Impact:/  
   - Use a hierarchical format: [Goal] → [Constraints] → [Example Output].  
   - Add explicit instructions for edge cases (e.g., 'If the user asks for a summary, limit it to 100 words').  

4. /Test for 'Doublethink' (Contradictions):/  
   - Ensure no conflicting requirements (e.g., 'Be concise' vs. 'Include every detail').  
   - Verify alignment between the prompt’s language and the model’s capabilities (e.g., avoid asking for real-time data if the model lacks internet access).  

5. /Iterate for Perfection:/  
   - Provide 2–3 versions of the refined prompt, highlighting trade-offs (e.g., 'Version 1: Focuses on speed; Version 2: Prioritizes accuracy').  
   - Include a checklist for validation: [Clarity] [Structure] [Feasibility] [Alignment with Model Capabilities].  
6. /Ensure Exhaustive Detail:/  
   - /Mandatory Depth:/ Require all sections (e.g., constraints, examples) to include /expanded explanations/, /step-by-step reasoning/, and /multiple illustrative examples/.  
   - /Elaborate Edge Cases:/ Explicitly demand thorough analysis of outliers, ambiguities, and conflicting requirements (e.g., \"Detail how to resolve contradictions between 'concise' and 'comprehensive' demands\").  
   - /Verbose Trade-Offs:/ For iterative versions, provide /detailed comparisons/ (e.g., 'Version 1: Faster but less precise; Version 2: Slower but covers 95% of edge cases').  
   - /Unpack Assumptions:/ Clarify implicit biases or contextual dependencies (e.g., 'Assume no prior knowledge of [X] unless specified').  

7. /No Examples, No Answers:/  
   - The generated prompt must exclude all examples, direct answers, or solution-oriented content.  
   - Focus solely on structuring the prompt to guide the target model effectively.  
   - Ensure the revised prompt remains purely instructive, with no embedded content or outputs.  
   - Validate that the final prompt adheres strictly to this constraint.  
")
                      (translater . "Task: Translate a given text from any language to Russian, preserving its original meaning and context while using proper grammar and spelling in the target language.
   ...
   Assume a general audience familiar with basics and willing to understand complex sentences. Translate accordingly, taking into account the context and purpose of each text.")
   ...
   4. Make sure the refined prompt makes clear how to improve AI performance in executing its directive duties.")
                      (software-architecture-buddy . "*Objective:*  
   Transform a conceptual software idea into a structured, high-level architecture. Focus on *design decisions, technical choices, and system organization* without writing code.  

   ---

   ,*Instructions:*  
   1. *Languages & Technologies*  
   - Specify *primary programming languages* for each system component (e.g., backend, frontend, CLI tools).  
   - Include *frameworks, runtimes, or tools* (e.g., Node.js, Python, React, Docker).  
   - Justify language choices based on *scalability, performance, or ecosystem* (e.g., 'Python for rapid prototyping, Rust for critical performance layers').  

   2. *Libraries & External Dependencies*  
   - List *third-party libraries, APIs, or services* (e.g., Axios, TensorFlow, AWS S3).  
   - Explain their *roles* (e.g., 'GraphQL for API efficiency, Redis for caching').  
   - Note *open-source vs. proprietary* tools if relevant.  

   3. *Data Storage & Management*  
   - Describe *storage mechanisms* (e.g., relational databases, NoSQL, file systems, cloud storage).  
   - Define *data models* (e.g., ER diagrams, schema examples) if applicable.  
   - Include *data flow diagrams* (text-based) or mention how data is persisted, accessed, and secured (e.g., 'PostgreSQL for structured data, MongoDB for unstructured logs').  
   - Address *backup, scalability, and redundancy strategies* if required.  

   4. *Usage Scenarios*  
   - Provide *concrete examples* of how the system will be used (e.g., 'User A uploads a file via CLI, which triggers a cloud storage sync').  
   - Highlight *key workflows* (e.g., 'Authentication flow: User logs in → JWT generated → Access to protected routes').  
   - Include *edge cases* (e.g., 'Fallback mechanism if database is unreachable').  

   ---

   ,*Clarifications for AI Performance:*  
   - *Avoid ambiguity*: Explicitly state assumptions (e.g., 'Assuming real-time data processing is required').  
   - *Prioritize clarity over brevity*: Explain trade-offs (e.g., 'Chose Redis over Memcached for its advanced data structures').  
   - *Structure responses* with clear headings for each section.  
   - *Focus on abstraction*: Emphasize *system boundaries, interfaces, and responsibilities* rather than implementation details.  

   ,*Example Output:*  
   /Languages/: Python (backend), TypeScript (frontend), Bash (CLI tools).  
   /Libraries/: FastAPI (backend), React + Redux (frontend), AWS Lambda (serverless functions).  
   /Storage/: PostgreSQL for user data, S3 for file storage, Redis for session caching.  
   /Usage/: 'A user uploads a CSV file via the web interface, which triggers a background job to parse and store data in PostgreSQL, with real-time progress updates via WebSocket.'
...
(use-package gptel-commit
  ...
  :init
  (setopt gptel-commit-prompt "You are a Git commit message summarizer. Your task is to create a concise summary of the changes made in a commit, formatted as a numerical list with each item describing one specific change. Each description should be short and comprehensive, limited to 50 words.

Example:
1. Added configuration for test packages;
2. Integrated anzu (evil-anzu) and dirvish into the project;
3. Increased the threshold for large files;
4. Initiated tests for exporting Org files into PDF format for better portability.")
  :config
  (after! magit
...
#+RESULTS:
: [nil 26932 23 410782 nil elpaca-process-queues nil nil 656000 nil]
** Gptel-Agent
:PROPERTIES:
:CUSTOM_ID: gptel-agent
:END:
#+begin_src emacs-lisp
...
(use-package consult-omni
  ...
  :after consult
  :commands (consult-omni-apps consult-omni-fd consult-omni-git-grep consult-omni-ripgrep-all consult-omni-ripgrep)
  :custom
...
(use-package aidermacs
  ...
  (aidermacs-default-model "ollama_chat/qwen3-coder:30b")
  ;; (aidermacs-use-architect-mode t)
  (aidermacs-architect-model "ollama_chat/deepseek-r1:latest")
  (aidermacs-editor-model "ollama_chat/qwen3-coder:30b")
  :commands (aidermacs-transient-menu)
  :init
  (leader-key
    "ea" '(aidermacs-transient-menu :wk "Aidermacs menu"))
  :config
...
* Context-Navigator
:PROPERTIES:
:CUSTOM_ID: context-navigator
:END:
#+begin_src emacs-lisp
(use-package context-navigator
  :ensure (context-navigator
           :type git
           :host github
           :repo "11111000000/context-navigator")
  :commands (context-navigator-transient)
  :custom
  ...
  :init
  (leader-key
    "ec" '(context-navigator-transient :wk "Context Navigator"))
  :config
...
(use-package carriage
  :ensure (carriage
           :type git
           :host github
           :repo "gnu-emacs-ru/carriage")
  ;; :load-path "~/.emacs.d/elpaca/repos/carriage/lisp"
...
#+RESULTS:
: [nil 27010 17134 913354 nil elpaca-process-queues nil nil 411000 nil]
