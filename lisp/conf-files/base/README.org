...
:END:
#+begin_src emacs-lisp
(defun get-cache-path (&optional cache-dir-name)
  "Return path to cache folder.
If we are in a project, folder is in root of project.
If not, it is in current directory.
CACHE-DIR-NAME is optional cache folder name (defaults to \".cached\")."
  (let ((cache-dir (or cache-dir-name ".cached"))
        (project-root (projectile-project-root))
        (current-file (buffer-file-name)))
    (if (and project-root
             (not (string-match-p (concat "^" (getenv "HOME") "/?$") project-root)))
        (progn
          (file-truename (concat project-root cache-dir)))
      (progn
        (if current-file
            (file-truename (concat (file-name-directory current-file) cache-dir))
          (file-truename (concat (getenv "HOME") "/" cache-dir )))))))
#+end_src
