...
#+STARTUP: content
#+PROPERTY: header-args :tangle base.el :results nil
* Содержание
:PROPERTIES:
...
- [[#emacs][Emacs]]
  - [[#cache-dir-function][Cache dir function]]
  - [[#emojis][Emojis]]
...
- [[#transient-postframe][Transient-Postframe]]
- [[#general-keybindigs][General Keybindigs]]
  - [[#transient-window][Transient-Window]]
  - [[#transient-toggle][Transient-Toggle]]
- [[#calc][Calc]]
- [[#wgrep][Wgrep]]
...
- [[#evil-mode][Evil-Mode]]
  ...
  - [[#evil-anzu][Evil-Anzu]]
  - [[#evil-better-visual-line][Evil-Better-Visual-Line]]
  - [[#evil-insert-plus][Evil-Insert-Plus]]
...
- [[#dired][Dired]]
  ...
  - [[#dired-du][Dired-DU]]
  - [[#dirvish][Dirvish]]
  - [[#dired-clipboard][Dired-Clipboard]]
...
** Cache dir function
:PROPERTIES:
:CUSTOM_ID: cache-dir-function
:END:
#+begin_src emacs-lisp
(defun get-cache-path (cache-dir-name full-path)
  "Return path to cache folder.
CACHE-DIR-NAME is the name of the cache folder (required).
If FULL-PATH is non-nil (default), return the full path (using file-truename).
Otherwise, return the path as is."
  (let ((project-root (projectile-project-root))
        (current-file (buffer-file-name)))
    (if full-path
        (if (and project-root
                 (not (string-match-p (concat "^" (getenv "HOME") "/?$") project-root)))
            (file-truename (concat project-root cache-dir-name))
          (if current-file
              (file-truename (concat (file-name-directory current-file) cache-dir-name))
            (file-truename (concat (getenv "HOME") "/" cache-dir-name))
            ))
      cache-dir-name)))
#+end_src
...
(defun init-text-scratch ()
  "Change mode in scratch buffer into text-mode"
  (remove-hook 'server-after-make-frame-hook 'init-text-scratch)
  (remove-hook 'after-init-hook 'init-text-scratch)
  (let ((original-buffer (current-buffer)))
    (with-current-buffer (get-buffer "*scratch*")
      (text-mode))))
(add-hook 'server-after-make-frame-hook 'init-text-scratch)
(add-hook 'after-init-hook 'init-text-scratch)
#+end_src
...
(use-package buffer-terminator
  ...
  :config
  (setq buffer-terminator-rules-alist '((keep-buffer-property . special) (keep-buffer-property . process)
                                        (keep-buffer-property . visible) (kill-buffer-property . inactive)
                                        (keep-buffer-name-regexp . ("\\` \\*ghostel\\*<?[0-9]*>?\\'") )))
...
(use-package general :ensure t :demand t
  ...
  (leader-key
    ...
    "i k" '(consult-yank-from-kill-ring :wk "Kill ring")
  ...
  (leader-key
    ...
    "f c" '((lambda () (interactive) (find-file "~/.emacs.d/lisp/conf-files/")) :wk "Edit config files")
  ...
  (leader-key
    ...
    "h E u" '((lambda () (interactive) (let ((current-prefix-arg '(4))) (call-interactively 'elpaca-update-menus))) :wk "Update all menus")
  ...
  ;;   "t f" `(focus-mode :wk "Focus")
  ;;   "t T" `(indent-tabs-mode :wk "Indent tabs")
  ;;   "t c" `(colorful-mode :wk "Colorful")
  ;;   )
...
** Transient-Toggle
:PROPERTIES:
:CUSTOM_ID: transient-toggle
:END:
#+begin_src emacs-lisp
(defun my/toggle--yasnippet-capf ()
  "Toggle yasnippet auto completion"
  (interactive)
  (if (memq 'yasnippet-capf completion-at-point-functions)
      (setq completion-at-point-functions
            (cl-remove 'yasnippet-capf completion-at-point-functions))
    (push 'yasnippet-capf completion-at-point-functions)))

(transient-define-prefix my/toggle-tmenu ()
  "Transient menu for managing togglable options"
  [["Toggle"
    ("l" display-line-numbers-mode
     :description (lambda ()
                    (format "Line Numbers %s"
                            (if display-line-numbers-mode "[x]" "[]")))
     :transient t)
    ("t" "Visual Line" visual-line-mode
     :description (lambda () (format "Visual Line %s"
                                     (if visual-line-mode "[x]" "[]")))
     :transient t)
    ("f" "Focus" focus-mode
     :description (lambda () (format "Focus %s"
                                     (if focus-mode "[x]" "[]")))
     :transient t)
    ("T" "Indent Tabs" indent-tabs-mode
     :description (lambda () (format "Indent Tabs %s"
                                     (if indent-tabs-mode "[x]" "[]")))
     :transient t)
    ("c" "Colorful" colorful-mode
     :description (lambda () (format "Colorful %s"
                                     (if colorful-mode "[x]" "[]")))
     :transient t)
    ("p" "Proxy" use-proxy-toggle-all-proxies
     :description (lambda ()
                    (format "Proxy %s"
                            (if (equal (use-proxy--proxy-states) "[]g")
                                "[]" "[x]")))
     :transient t)]
   ["Completion"
    ("y" my/toggle--yasnippet-capf
     :description (lambda () (format "Yasnippet CAPF %s" (if (memq 'yasnippet-capf
...
(use-package evil
  ...
  (evil-mode)
  ;; По-другому расположить evil-insert-plus не получилось, иначе жалуется, что загружен макрос
  (use-package evil-insert-plus
  :commands (evil-insert-plus--vcount)
...
#+RESULTS:
: [nil 26896 32197 152189 nil elpaca-process-queues nil nil 537000 nil]

** Evil-Better-Visual-Line
:PROPERTIES:
:CUSTOM_ID: evil-better-visual-line
:END:
#+begin_src emacs-lisp
...
(use-package vterm
  ...
  (general-define-key
   ...
   "m c" '(vterm-copy-mode :wk "Toggle copy mode")
   )
  :config
...
(use-package ghostel
  ...
  :bind (("C-x m" . ghostel)
         ...
         ("C-d" . my/ghostel-send-C-d-and-kill)
  ...
  :commands (ghostel-spawn my/ghostel-bottom-window ghostel-other)
  :init
  (defun ghostel-spawn (&optional args)
    (let ((name (or args ""))
          ;; (ghostel-exit-functions '(kill-buffer-and-window))
          )
      ;; (message name)
      (if (not (string-empty-p name))
          (ghostel name)
  ...
  (defun my/ghostel-bottom-window (number)
    (interactive "p")
    (let ((display-buffer-alist '(("^\\*ghostel" . (display-buffer-in-side-window
                                                     (side . bottom)
                                                     (slot . 0)
                                                     (window-height . 0.35))))))
      (ghostel number)))
  ...
  (leader-key
    "o t" '(my/ghostel-bottom-window :wk "Opens Ghostel in other window")
    "o T" '(ghostel-other :wk "Open Ghostel instead of this buffer")
    )
  :config
...
Like normal Emacs `C-k'.  Kill to end of line and put content in kill-ring."
  ...
  (defun my/ghostel-send-C-d-and-kill ()
    "Send `C-k' to ghostel.
Like normal Emacs `C-k'.  Kill to end of line and put content in kill-ring."
    (interactive)
...
#+RESULTS:
: [nil 27197 26275 880964 nil elpaca-process-queues nil nil 927000 nil]
* Which-key
:PROPERTIES:
:CUSTOM_ID: which-key
:END:
...
#+RESULTS:
: [nil 27308 10328 146439 nil elpaca-process-queues nil nil 603000 nil]
...
#+RESULTS:
: [nil 26913 20270 96909 nil elpaca-process-queues nil nil 101000 nil]
** Dired-Clipboard
:PROPERTIES:
:CUSTOM_ID: dired-clipboard
:END:
#+begin_src emacs-lisp
(use-package dired-clipboard
  :ensure (dired-clipboard :type git :host github :repo "kn66/dired-clipboard.el" :branch "main"))
#+end_src

#+RESULTS:
: [nil 27198 41758 295587 nil elpaca-process-queues nil nil 965000 nil]
...
(use-package password-store-menu
  ;; :custom
  ;; (password-store-menu-key  "SPC sP" "Menu Key for password-store")
  ;; :config (password-store-menu-enable)
  :general
  (:states 'normal "SPC s P" 'password-store-menu)
  )
;; (general-auto-unbind-keys)
#+end_src
